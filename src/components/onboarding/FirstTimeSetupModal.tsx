...
export default function FirstTimeSetupModal() {
  const { defaultRegion, setDefaultRegion, hasCompletedOnboarding, completeOnboarding } = useSettingsStore();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-2xl mx-4 bg-lol-card border border-lol-border rounded-2xl shadow-2xl shadow-black/50">
        ...
        <div className="px-8 pt-8 pb-4 text-center">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-lol-gold-light to-lol-gold flex items-center justify-center text-lol-dark font-bold text-2xl shadow-lg shadow-lol-gold/20">
            TC
          </div>
          <h1 className="text-2xl font-bold text-white mb-2">Welcome to Teamcomp.lol</h1>
          <p className="text-gray-400">Select your default server region to get started</p>
        </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {group.regions.map((region) => (
                  ...
                  >
                    ...
                    <div className={`text-xs mt-0.5 ${selectedRegion === region ? 'text-lol-gold/70' : 'text-gray-500'}`}>
