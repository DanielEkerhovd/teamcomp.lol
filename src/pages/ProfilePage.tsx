    ...
    <div className="group">
      <div className="flex items-center gap-4 p-4 rounded-xl hover:bg-lol-surface/50 transition-colors -mx-4">
        ...
        <div className="relative shrink-0">
          ...
          >
            {avatarUrl ? (
              ...
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-lol-gold to-lol-gold-light flex items-center justify-center text-lol-dark font-bold text-base">
                {initials}
              </div>
            )}
            {/* Hover overlay */}
        ...
        <div className="flex-1 min-w-0">
          <div className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">User</div>
          {isEditing ? (
            ...
            <>
              ...
              <div className="text-xs text-gray-500 mt-0.5">This is how others will see you</div>
            </>
          )}
        </div>
        {/* Edit button */}
...
export default function ProfilePage() {
  ...
  const { user, profile, isLoading, signOut, updateDisplayName, updateAvatar, removeAvatar, deleteAccount } = useAuthStore();
  ...
  const handleDeleteAccount = async () => {
    ...
    if (result.error) {
      setDeleteError(`${result.error}. If this issue persists, please contact support@teamcomp.lol`);
      setIsDeleting(false);
    } else {
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-lol-card border border-lol-border rounded-2xl p-8 text-center mb-6">
          ...
          <p className="text-gray-400 mb-6">Sign in to access your profile and sync your data across devices.</p>
          <button
  ...
  // Authenticated view
  const initials = (profile?.displayName || user.email?.split('@')[0] || 'U').slice(0, 2).toUpperCase();
  const tierBadge = profile?.tier === 'paid'
    ...
    ? 'Admin'
    : 'Free';
  const tierBadgeColor = profile?.tier === 'paid'
      ...
      <div className="bg-lol-card border border-lol-border rounded-2xl p-6 mb-6">
        ...
        <div className="p-5 rounded-xl bg-lol-surface/50 border border-lol-border mb-6">
          <div className="flex items-center justify-between mb-4">
            <span className="text-gray-400 text-sm uppercase tracking-wide">Current Plan</span>
            <span className={`px-4 py-1.5 text-base font-bold rounded-full ${tierBadgeColor}`}>
              {tierBadge}
            </span>
          </div>
          <ul className="space-y-2 text-sm text-gray-300">
            <li className="flex items-center gap-2">
              ...
              <span>Manage {profile?.maxTeams ?? 1} team{(profile?.maxTeams ?? 1) !== 1 ? 's' : ''}</span>
            </li>
            <li className="flex items-center gap-2">
              <svg className="w-4 h-4 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            ...
            {(profile?.tier === 'paid' || profile?.tier === 'supporter' || profile?.tier === 'admin') && (
              <li className="flex items-center gap-2">
                <svg className="w-4 h-4 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
        ...
        {(profile?.tier === 'free' || profile?.tier === 'paid') && (
          <>
            <div className="text-gray-400 text-sm uppercase tracking-wide mb-4">Upgrade Options</div>
            <div className="grid grid-cols-2 gap-4">
              {/* Pro Tier */}
              {profile?.tier === 'free' && (
                <div className="p-4 rounded-xl border border-lol-border bg-lol-surface/30 hover:border-lol-gold/30 transition-colors flex flex-col">
                  ...
                  <ul className="space-y-2 text-sm text-gray-300 flex-1">
                    <li className="flex items-center gap-2">
                      ...
                      <span className="text-white font-medium">Unlimited drafts</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <svg className="w-4 h-4 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  ...
                  <button className="w-full py-2 mt-4 bg-lol-gold text-lol-dark font-medium rounded-lg hover:bg-lol-gold-light transition-colors">
                    Upgrade to Pro
                  </button>
                </div>
              )}
              ...
              {/* Supporter Tier */}
              <div className={`p-4 rounded-xl border relative overflow-hidden border-purple-500/30 bg-lol-surface/30 hover:border-purple-500/50 transition-colors ${profile?.tier === 'free' ? '' : 'col-span-2'}`}>
                ...
                <ul className="space-y-2 text-sm text-gray-300 mb-4">
                  <li className="flex items-center gap-2">
                    ...
                    <span>{profile?.tier === 'free' ? 'Everything in Pro' : 'Unlimited teams'}</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <svg className="w-4 h-4 text-purple-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                  ...
                  <li className="flex items-center gap-2">
                    ...
                    <span>Supporter badge on Discord</span>
                  </li>
                </ul>
